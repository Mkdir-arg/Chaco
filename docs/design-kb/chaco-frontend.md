...
- **CSS legacy global con `!important`:** hay CSS viejo (nodo-brand.css, override.css, etc.) que pisa estilos —incluso inline— en componentes compartidos (sidebar, navbar). Si un color/fondo no “pega”, **blindá** el componente: poné una **clase wrapper** y un `<style>` **inline en el HTML** (no en un .css, que además se cachea) que fuerce los valores por **token + `!important`**, idealmente state-driven por `[aria-current="page"]`. Ej. sidebar: `.ds-snav a{color:var(--text-body)!important} .ds-snav [aria-current="page"]{background:var(--bg-brand)!important;color:#fff!important}`.
  ...
  - **⚠ Landmine global de links:** `nodo-brand.css` tiene `a:not(.btn):not(.btn-nodo):not(.btn-brand):not([class*="bg-"]) { color: var(--nodo-magenta) !important; }` (especificidad **0,4,1**) → pinta de **magenta TODO `<a>`** de la app (texto que sea nodo directo del link). Aparece en CUALQUIER pantalla con links. Para vencerlo, replicá su patrón con tu clase wrapper delante (espec. mayor), p.ej.: `.tuscope a:not(.btn):not(.btn-nodo):not(.btn-brand):not([class*="bg-"]) { color: var(--text-body) !important; }`. (Solución de fondo pendiente: neutralizar/scopear esa regla en nodo-brand.css para todo el proyecto.)
